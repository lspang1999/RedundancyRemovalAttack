...
parser = argparse.ArgumentParser("Add camoflauge to a random gate")
...
# select random gate to camo
selected_gate = random.sample(gateNodes.keys(), 1)
selected_gate = selected_gate[0]
# check that gate is and or or:
print('Entering loop')
while 'and' != InstanceListDict[selected_gate].module and 'or' != InstanceListDict[selected_gate].module and \
        len(gateNodes[selected_gate]) != 3:
    selected_gate = random.sample(gateNodes.keys(), 1)[0]
print('Exiting loop')
print(selected_gate)

# replace gate with camo gate
new_instance_dict = {}
for gate in gateNodes.keys():
    if gate == selected_gate:
       # change gate type to camo
       oldGatePorts = gateNodes[gate]
       newInst = vast.Instance("camo", gate, [vast.PortArg(None, vast.Identifier(oldGatePorts[0])),
